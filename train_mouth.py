...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    motion_net = MouthMotionNetwork().cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if not viewpoint_stack:
            viewpoint_stack = scene.getTrainCameras().copy()    #len = 7272
        ...
        if iteration < warm_step:
            ...
            render_pkg = render_motion_mouth(viewpoint_cam, gaussians, motion_net, pipe, background,ite=iteration)
        ###########################################################
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if renderFunc is render:
                        ...
                        render_pkg = renderFunc(viewpoint, scene.gaussians, motion_net, *renderArgs,ite=iteration)
