...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".jpg", audio_file='', audio_extractor='deepspeech', preload=True):
    ...
    if transformsfile == "transforms_train.json":
        if os.path.exists(os.path.join(path,'train_auds_exp.pth')):
            auds_exp = torch.load(os.path.join(path,'train_auds_exp.pth'))
        else:
            assert False, "Extral info file: <train_auds_exp.pth> do not exist!"

        if os.path.exists(os.path.join(path,'train_audiostyle.pth')):
            audiostyle = torch.load(os.path.join(path,'train_audiostyle.pth'))
        else:
            assert False, "Extral info file: <train_audiostyle.pth> do not exist!"

        if os.path.exists(os.path.join(path,'mser_emotion_train.pth')):
            emotion_deature = torch.load(os.path.join(path,'mser_emotion_train.pth'))
        else:
            assert False, "Extral info file: <mser_emotion_train.pth> do not exist!"

    elif transformsfile == "transforms_val.json":
        if os.path.exists(os.path.join(path,'val_auds_exp.pth')):
            auds_exp = torch.load(os.path.join(path,'val_auds_exp.pth'))
        else:
            assert False, "Extral info file: <val_auds_exp.pth> do not exist!"

        if os.path.exists(os.path.join(path,'val_audiostyle.pth')):
            audiostyle = torch.load(os.path.join(path,'val_audiostyle.pth'))
        else:
            assert False, "Extral info file: <val_audiostyle.pth> do not exist!"

        if os.path.exists(os.path.join(path,'mser_emotion_val.pth')):
            emotion_deature = torch.load(os.path.join(path,'mser_emotion_val.pth'))
        else:
            assert False, "Extral info file: <mser_emotion_val.pth> do not exist!"

    else:
        assert False, "Wrong transforms file!!!"
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames)):
            ...
            talking_dict['auds_exp'] = auds_exp[idx]
            talking_dict['audio_style'] = audiostyle[idx]
            talking_dict['emotion'] = emotion_deature[idx]
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".jpg", args=None):
    ...
    if not eval:
        ...
        #train_cam_infos = [CameraInfo,CameraInfo,CameraInfo,CameraInfo,...]
        # CameraInfo = {}
