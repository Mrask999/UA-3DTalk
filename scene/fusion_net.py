...
class fusion_net(nn.Module):
    def __init__(self, in_dim:list, hidden_dim:int, latent_dim:int, T:int, xyz_dim:int, feature_dim:int):
        ...
        self.emotion_net = nn.ModuleList([uncertainty_net(in_dim[3], hidden_dim, latent_dim) for _ in range(T)])

        self.fc = nn.Linear(latent_dim + xyz_dim, feature_dim)


    def forward(self, audio, content, style, emotion, xyz, stage):
        audio = torch.cat([audio, xyz], dim=-1)
        content = torch.cat([content, xyz], dim=-1)
        style = torch.cat([style, xyz], dim=-1)

        if stage == 1:
            model_index = [random.randint(0,self.T-1) for _ in range(self.net_num)]
            ...
            emotion_mu, emotion_var = self.emotion_net[model_index[3]](emotion)
            ...
            emotion_var = torch.diag_embed(torch.exp(emotion_var))
            ...
            for i in range(self.T):
                ...
                tmp = self.emotion_net[i](emotion)
                emotion_mu.append(tmp[0])
                emotion_var.append(tmp[1])
            ...
            emotion_mu = torch.stack(emotion_mu, dim=0).transpose(1, 0)
            ...
            emotion_var = torch.exp(torch.stack(emotion_var, dim=0)).mean(dim=0,keepdim=False)
            assert audio_var.shape == torch.Size([xyz.shape[0],self.latent_dim])
            assert content_var.shape == torch.Size([xyz.shape[0],self.latent_dim])
            assert style_var.shape == torch.Size([xyz.shape[0],self.latent_dim])
            assert emotion_var.shape == torch.Size([xyz.shape[0],self.latent_dim])
            ...
            emotion_var = batch_covariance(emotion_mu) + torch.diag_embed(emotion_var)
            ...
            emotion_mu = emotion_mu.mean(dim=1, keepdim=False)
        ...
        emotion_var_inverse = torch.inverse(emotion_var)
        fused_var = torch.inverse(audio_var_inverse + content_var_inverse + style_var_inverse + emotion_var_inverse)
        tmp = (torch.bmm(audio_var_inverse, audio_mu.unsqueeze(2)) +
               ...
               torch.bmm(emotion_var_inverse, emotion_mu.unsqueeze(2)) )
