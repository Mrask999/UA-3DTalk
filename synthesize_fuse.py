...
from scene.deformation import MotionNetwork,MouthMotionNetwork
...
def render_set(model_path, name, iteration, views, gaussians, motion_net, gaussians_mouth, motion_net_mouth, pipeline, background, fast, dilate):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress", ascii=True)):
        ...
        with torch.no_grad():
            render_pkg = render_motion(view, gaussians, motion_net, pipeline, background, frame_idx=0,ite=99999)
            render_pkg_mouth = render_motion_mouth(view, gaussians_mouth, motion_net_mouth, pipeline, background, frame_idx=0,ite=99999)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, use_train : bool, fast, dilate):
    with torch.no_grad():
        ...
        motion_net = MotionNetwork().cuda()
        motion_net_mouth = MouthMotionNetwork().cuda()
